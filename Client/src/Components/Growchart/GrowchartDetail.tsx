...
function interpolate(data: RefPoint[], x: number): Omit<RefPoint,"x"> | null {
  ...
  if (x >= sorted[sorted.length-1].x) {
    ...
    // We linearly scale the curve up to 5 years (300 weeks CGA) 
    // This creates a smooth continuous line instead of flatlining or creating stairs
...
function cgaWeek(dob: string, gaAtBirth: number, visitDate: string): number {
  return gaAtBirth + (new Date(visitDate).getTime() - new Date(dob).getTime()) / (7*24*3600*1000);
}
...
function buildData(
  visits: { date:string; height:string; weight:string; headCirc:string; gaWeeks:string }[],
  ...
  visits.filter(v => v.date && dob).forEach(v => {
    const cga = v.gaWeeks && parseFloat(v.gaWeeks) > 0 ? parseFloat(v.gaWeeks) : cgaWeek(dob, gaAtBirth, v.date);
    ...
    const idx = pts.findIndex(r => Math.abs(r.week - vp.week) < 2.0 && r.patient === null);
...
const percentileLabelsPlugin = {
  ...
  afterDraw(chart: any) {
    ...
    // Collect label positions for all non-patient datasets
    ...
    chart.data.datasets.forEach((dataset: any) => {
      ...
      // Find the last point that is within the visible x range
      ...
      // Pin label to the right edge of the chart area
      const labelX = Math.min(rawX, xRight - 2);
      labelItems.push({ label: pctLabel, color: dataset.borderColor, x: labelX, y: rawY });
    });

    if (!labelItems.length) return;

    // Sort by y ascending (top to bottom) for collision resolution
    ...
    // Backward pass: push labels up if too close (handles bottom overflow)
    const yBottom = chartArea.bottom - 4;
    for (let i = labelItems.length - 1; i >= 0; i--) {
      if (labelItems[i].y > yBottom) labelItems[i].y = yBottom;
      if (i > 0 && labelItems[i - 1].y > labelItems[i].y - minGap) {
        labelItems[i - 1].y = labelItems[i].y - minGap;
      }
    }
    ...
    labelItems.forEach(({ label, color, x, y }) => {
      ctx.fillStyle = color;
...
function MetricChart({ data, yLabel, yStep, yMin, genderView, gaAtBirth }: {
  ...
  // Post-birth: 2m, 4m, 6m, 8m, 10m then yearly for 5 years
  for (let y = 0; y < 5; y++) {
    const base = gaAtBirth + (y * 52);
    customTicks.push(
      { value: base + 8.67 },
          ...
          <div style={s.chartCardHeader}>
            ...
            <div style={s.inlineLegend}>
              ...
              {(genderView === "both" || genderView === "female") && (
                <span style={s.legendItem}>
                  <span style={{ ...s.legendLine, borderColor: GIRLS_COLORS[2] }} />
                  Girls (3rd–97th)
                </span>
              )}
              <span style={s.legendItem}>
